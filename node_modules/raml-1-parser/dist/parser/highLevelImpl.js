    ...
    ASTNodeImpl.prototype.types = function (requestedByFragmentOrLibrary) {
        ...
        if (!this._types) {
            ...
            if (unit && this.parent() && this.definition() && !isLibrary) {
                ...
                var includePath = this.lowLevel().includePath();
                var included = !isInLibExpandMode && includePath != null;
                var parentTypes = this.parent().types();
                var thisDef = this.definition();
                if (!included || !thisDef || !universeHelpers.canBeFragment(thisDef)) {
                    return parentTypes;
                }
                var iUnit = unit.resolve(includePath);
    ...
    ASTNodeImpl.prototype.parsedType = function () {
        ...
        if (!this._ptype) {
            ...
            if ((!isApi || isInLibExpandModel) && this.definition().isAssignableFrom(universes.Universe10.TypeDeclaration.name)) {
                ...
                if (parent_2 && parent_2.definition().isAssignableFrom(universes.Universe10.TypeDeclaration.name)) {
                    var parentType = parent_2.parsedType();
                    if (parentType != null) {
                ...
                else if (parent_2 && parent_2.definition().isAssignableFrom(universes.Universe10.LibraryBase.name)) {
